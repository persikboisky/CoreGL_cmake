...
namespace core
{
	namespace vulkan
	{
		...
		void Queue::submit(const queueSubmitInfo* qs)
		{
			...
			submitInfo.pWaitSemaphores = qs->waitPtrSemaphore->getVkPtrSemaphore();
			submitInfo.pWaitDstStageMask = waitStages;
			submitInfo.commandBufferCount = 1;
			submitInfo.pCommandBuffers = qs->PtrCommandBuffer->getVkPtrCommandBuffer();

			submitInfo.signalSemaphoreCount = 1;
			submitInfo.pSignalSemaphores = qs->finishSignalSemaphore->getVkPtrSemaphore();
		...
		void Queue::present(const queuePresentInfo* qp)
		{
			...
			presentInfo.pWaitSemaphores = qp->waitPtrSemaphore->getVkPtrSemaphore();
