...
namespace core
{
    namespace vulkan
    {
        VkColorSpaceKHR PhysicalDeviceInfo::getColorSpace(const Surface &surface, const IMAGE_FORMAT &imageFormat) const
        {
            uint32_t count = 0;
            ...
            for (const auto& surfaceFormat: surfaceFormats)
                if (surfaceFormat.format == Convert::convert(imageFormat))
        ...
        PhysicalDeviceInfo::PhysicalDeviceInfo(const VkPhysicalDevice& physicalDevice) : physicalDevice(physicalDevice)
        {
        ...
        void PhysicalDeviceInfo::printQueueFamilyInfo()
        {
            ...
            for (uint32_t i = 0; i < queueFamilies.size(); i++)
            {
                ...
                std::cout << "TimestampValidBits: " << queueFamilies[i].timestampValidBits << std::endl;
        ...
        uint32_t PhysicalDeviceInfo::getMaxPushConstantsSize() const
        {
        ...
        bool PhysicalDeviceInfo::checkFormatSupport(const class Surface &surface, const IMAGE_FORMAT &format) const
        {
            uint32_t count = 0;
            std::vector<VkSurfaceFormatKHR> surfaceFormats = {};
            VkResult result = vkGetPhysicalDeviceSurfaceFormatsKHR(
            ...
            for (const auto& surfaceFormat: surfaceFormats)
                if (surfaceFormat.format == Convert::convert(format))
                    return true;
