...
namespace core
{
	namespace vulkan
	{
		CommandPool::CommandPool(const CommandPoolCreateInfo& info) : ptrDevice(&info.ptrDevice->device)
		{
		...
		CommandPool CommandPool::create(const CommandPoolCreateInfo& info)
		{
			return CommandPool(info);
		}

		CommandPool* CommandPool::ptrCreate(const CommandPoolCreateInfo& info)
		{
