...
namespace core
{
	namespace vulkan
	{
		static inline VkBufferUsageFlags convertBufferType(const TYPE_USAGE_BUFFER& type)
		{
			...
			case TYPE_USAGE_BUFFER::VERTEX_TRANSFER_SRC:
			    return VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
			case TYPE_USAGE_BUFFER::INDEX_TRANSFER_SRC:
		...
		Buffer::Buffer(const BufferCreateInfo& info) : ptrDevice(&info.ptrDevice->device), size(info.size)
		{
			...
			bufferInfo.queueFamilyIndexCount = static_cast<uint32_t>(info.queueFamilyIndices.size());
		...
		void Buffer::copy(const void * data, uint64_t size, uint64_t offset) const
        {
			void* ptrMemory;
