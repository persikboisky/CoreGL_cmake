...
namespace core
{
	namespace lua
	{
		...
		Array::num Array::getNumber(int index) const
        {
			if (nFree == 0)
		...
		Array::str Array::getString(int index) const
        {
			if (nFree == 0)
				stack->pushFromGlobal(this->name);
			stack->pushFromGlobalTable(index);
		...
		int Array::getInteger(int index) const
        {
			if (nFree == 0)
				stack->pushFromGlobal(this->name);
			stack->pushFromGlobalTable(index);
		...
		bool Array::getBoolean(int index) const
        {
			if (nFree == 0)
				stack->pushFromGlobal(this->name);
			stack->pushFromGlobalTable(index);
